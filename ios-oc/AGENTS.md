...
1. Agent REST API (start/stop agent) — defined in `Tools/AgentManager.m`:
...
2. Token generation service — defined in `Tools/AgentManager.m` → `generateTokenWithChannelName:uid:types:success:`:
...
To point to your own backend: modify the URL strings in `AgentManager.m`. If your backend handles auth differently, also update the generated request headers there.

If using a local server, the host must be the IP address of the machine running the server, not `localhost` or `127.0.0.1`. The iOS device and the server must be on the same local network.

⚠️ Security warning: This demo embeds all API keys (LLM, TTS) directly in the client-side request body for quick demonstration and debugging. For production use, move those keys to your own backend service.
...
- Add/update the corresponding API key in `KeyCenter.m`
...
To modify request parameters: edit the `startAgentParameter` dictionary. Static values should stay in `KeyCenter.m`; structural changes go directly in the dictionary.
