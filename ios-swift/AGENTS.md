...
Static credentials are read directly from `KeyCenter.swift`. `ViewController` uses them to build the start-agent payload, while `NetworkManager` uses `APP_ID` + `APP_CERTIFICATE` for demo token generation.
...
8. ConversationalAIAPI receives agent state / transcript events via RTM → `ViewController` updates UI
