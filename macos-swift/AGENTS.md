...
1. Agent REST API (start/stop agent) — defined in `VoiceAgent/API/AgentManager.swift`:
...
To point to your own backend: modify the URL strings in `AgentManager.swift` and `TokenGenerator.swift`. If your backend handles auth differently, also update the generated request headers.
...
2. `AgentManager.swift` → `buildPayload(...)` — the parameter dictionary that specifies vendor names and maps `KeyCenter` values into the request body
...
The agent start request body is built in `AgentManager.swift` → `buildPayload(...)` as a nested dictionary. Key sections:
...
To modify request parameters: edit the payload in `AgentManager.swift`. Static values should stay in `KeyCenter.swift`; structural changes go directly in the payload.

## What NOT to Modify

The `VoiceAgent/ConversationalAIAPI/` directory is a standalone reusable component. Do not modify it. See `ConversationalAIAPI/README.md` for integration details.
